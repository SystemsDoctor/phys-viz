...
M0-G is now met — every task below is `READY` to start.

- [READY] **M1-1** `kernel/math` — Vec2/Vec3/Mat3/Mat4/Quat as plain typed structures with free functions, providing **both** allocating and in-place (`addInto(out, a, b)`) variants (`src/kernel/math/index.ts` stubs all throw)
- [READY] **M1-2** `kernel/math` scratch pool — a pre-allocated ring of temporaries (`tmp.v3()`), documented loudly, with a test proving a hot path through it allocates nothing. GC pauses are visible as stutter on a projector (§7)
- [READY] **M1-3** `kernel/math` quaternion path complete enough that no caller ever needs Euler storage — the rigid-body and gyroscope modules gimbal-lock otherwise (§7). Include quat↔matrix, slerp, `fromAxisAngle`
- [READY] **M1-4** `kernel/frames` — Cartesian ↔ cylindrical ↔ spherical conversions **with Jacobians**, right-handed per ADR 0008: cylindrical `(r, θ, z)` with `θ` from `+x` toward `+y`, spherical `(r, θ, φ)` in the physics convention. The handedness fixes the sign of every off-diagonal Jacobian term
- [READY] **M1-5** `kernel/frames` — `Frame` type (origin + orientation + optional ω and ω̇), `transformPoint`, `transformVector`
- [READY] **M1-6** `kernel/frames` — `transformVelocity` / `transformAcceleration` exposing ω × r, 2ω × v (Coriolis) and ω × (ω × r) (centrifugal) as **separately retrievable components**, not just a sum. The non-inertial-frames module draws each term as its own arrow (§7)
- [READY] **M1-7** `kernel/calculus` — `grad`/`div`/`curl` by central differences with an adaptive `h`
- [READY] **M1-8** `kernel/calculus` — `lineIntegral`/`surfaceFlux`/`volumeIntegral` on **Gauss–Legendre** quadrature, not naive Riemann sums
- [READY] **M1-9** `kernel/calculus` — every integral returns the value **and** the per-sample contributions, so the visualizer can shade an accumulating ribbon rather than just report a number (§7; consumed by M5-4)
- [READY] **M1-10** `kernel/geometry` — signed polygon area and centroid (shoelace)
- [READY] **M1-11** `kernel/geometry` — Sutherland–Hodgman clip of a polygon by a half-plane. Build early per §7 even though no M4/M5 module needs it: it is the foundation of every submerged-area / cross-section / cutaway module later (ship stability, buoyancy, beam sections). Cheap now, unblocking later
- [READY] **M1-12** `kernel/geometry` — 2D convex hull, point-in-polygon, ray–plane and ray–sphere intersection (the last two feed M2 picking)
- [READY] **M1-13** `kernel/ode` — `rk4`, `velocityVerlet` (symplectic; the default for anything conservative), `rkf45` (adaptive). All pure `(state, dt) => state` and allocating nothing — assert the zero-allocation property in a test
- [READY] **M1-14** `kernel/ode` — event detection: root-find a scalar event function between steps (bisection is fine) for turning points, zero-crossings, and "hits the ground"
- [READY] **M1-15** `kernel/ode` — a general scalar root-finder (Newton–Raphson with a bisection fallback). §12 assumes one exists ("a Kepler orbit via Newton–Raphson on Kepler's equation") and E-2's linkage solver reuses it. Not named in the §5 file list; put it in `ode/` and note the addition
- [READY] **M1-16** `kernel/units` — `Quantity { value, dim }` over the `[M, L, T, Θ, I, N, J]` exponent tuple, with arithmetic that checks dimensions and throws on mismatch
- [READY] **M1-17** `kernel/units` — formatting with SI prefixes and significant-figure control (this makes readouts and axis labels correct for free, §7). Output must be fixed-width so live readouts don't jitter as digits change, pairing with Plex Mono's tabular figures (§15)
- [READY] **M1-18** `kernel/expr` — recursive-descent parser over the whitelisted grammar (numbers, named vars, `+ - * / ^`, parens, and exactly the function set in §7), compiling to a closure. **No `eval`, no `new Function`** — this is user-facing input on a public site
- [READY] **M1-19** `kernel/expr` — typed errors with character offsets so `ExpressionField` can underline the problem (§7, §9)
- [READY] **M1-20** `kernel` seeded PRNG. §12's determinism requirement permits module randomness only via "a seeded PRNG from the kernel, seeded from a serialized param", but §5 gives it no home. Add it (`kernel/random`, re-exported from `kernel/index.ts`) and record the §5 addition. Contract assertion 9's quasi-random parameter sampling also needs it
- [READY] **M1-21** `kernel` inertia tensors for the schematic body set (box, sphere, cylinder, disc, rod) plus the parallel-axis theorem. §18's golden-value list names "the inertia tensor of a uniform cuboid", so this must exist in the kernel at M1 — and M5-1 needs it regardless
- [READY] **M1-22** `kernel` symmetric 3×3 eigendecomposition. §22 states it is "already needed for the inertia tensor in M5" (inertia ellipsoid, principal axes) and it later unlocks Mohr's circle. Pure arithmetic; belongs here, not in a module
...
- [READY] **M1-24** Wire the ≥90% coverage threshold into CI (depends on M0-7) so the M1 gate is machine-checked rather than asserted
- [READY] **M1-25** Prove the purity half of the gate: no file under `src/kernel/` imports anything outside it, and `npm run lint` fails if one does (M0-9 closes the `three/*` hole that would otherwise let a violation pass)
- [READY] **M1-G** **Gate:** ≥90% kernel coverage, all golden-value tests pass, kernel purity enforced by lint. Do not start M2 until this is true
