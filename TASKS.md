...
- [READY] **M0-14** Push to GitHub, confirm `deploy.yml` publishes to Pages at the live URL, confirm `ci.yml` is green on the PR. In progress: pushed the M0-6…M0-13 commits, then found both workflows in `disabled_manually` state on GitHub (from the pre-lockfile pushes that failed instantly at `npm ci`) — no run exists yet against the current HEAD. Re-enabled both (`gh workflow enable`); this commit's push is what actually exercises them
