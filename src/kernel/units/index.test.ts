...
describe('formatQuantity', () => {
  ...
  it('formats zero', () => {
    expect(U.formatQuantity({ value: 0, dim: LENGTH }).trim()).toBe('0.00');
  });

  it('picks the kilo prefix for thousands', () => {
    const s = U.formatQuantity({ value: 4560, dim: LENGTH });
    expect(s.trim()).toBe('4.56k');
  });

  it('picks the milli prefix for small values', () => {
    const s = U.formatQuantity({ value: 0.00456, dim: LENGTH });
    expect(s.trim()).toBe('4.56m');
  });

  it('picks the micro prefix', () => {
    const s = U.formatQuantity({ value: 0.0000123, dim: LENGTH });
    expect(s.trim()).toBe('12.3µ');
  });

  it('prefixes negative values with a minus sign', () => {
    const s = U.formatQuantity({ value: -4.56, dim: LENGTH });
  ...
  it('is fixed-width across a range of magnitudes for a fixed sigFigs', () => {
    ...
    for (const value of [1, 12, 123, 1234, 12345, 0.1, 0.01, 0.001, 999, 999999]) {
      widths.add(U.formatQuantity({ value, dim: LENGTH }, 3).length);
    }
    expect(widths.size).toBe(1);
  });

  it('respects a custom sigFigs count', () => {
    const s = U.formatQuantity({ value: 4.5678, dim: LENGTH }, 5);
    expect(s.trim()).toBe('4.5678');
  });

  it('renders engineering-notation groups correctly (2 and 3 integer digits before the decimal)', () => {
    expect(U.formatQuantity({ value: 45.6, dim: LENGTH }).trim()).toBe('45.6');
    expect(U.formatQuantity({ value: 456, dim: LENGTH }).trim()).toBe('456');
  });
  ...
  it('corrects a Math.log10 rounding artifact just under a power-of-1000 boundary', () => {
    ...
    const s = U.formatQuantity({ value: 999999.9999999999, dim: LENGTH });
  ...
  describe('DIMENSIONLESS quantities', () => {
    it('never shows an SI-prefix letter for a pure ratio in (0, 1)', () => {
      // Regression: a direction cosine like 0.949 previously rendered as
      // "949m" — the "milli" prefix picked by engineering notation, which
      // is meaningless for a dimensionless quantity.
      const s = U.formatQuantity({ value: 0.949, dim: U.DIMENSIONLESS });
      expect(s.trim()).toBe('0.949');
      expect(s.trim()).toMatch(/^-?\d+\.\d+$/);
    });

    it('prints a plain decimal for a negative ratio in (-1, 0)', () => {
      const s = U.formatQuantity({ value: -0.5, dim: U.DIMENSIONLESS });
      expect(s.trim()).toBe('-0.500');
    });

    it('still formats zero and values >= 1 with no prefix', () => {
      ...
      expect(U.formatQuantity({ value: 4560, dim: U.DIMENSIONLESS }).trim()).toBe('4560');
    });

    it('respects a custom sigFigs count for values below 1', () => {
      const s = U.formatQuantity({ value: 0.0012345, dim: U.DIMENSIONLESS }, 5);
      expect(s.trim()).toBe('0.0012345');
    });

    it('corrects when rounding pushes a sub-1 ratio up to the next order of magnitude', () => {
      // 0.9996 rounds to "1.000" at 3 decimals, which must be re-derived
      // with one fewer decimal now that the value's order of magnitude
      // changed from -1 to 0.
      const s = U.formatQuantity({ value: 0.9996, dim: U.DIMENSIONLESS });
      expect(s.trim()).toBe('1.00');
    });
  });
});
