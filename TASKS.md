...
- [READY] **M2-1** `Viewport` — renderer, canvas, resize observer, and **exactly one** `requestAnimationFrame` loop for the whole app (§8)
- [READY] **M2-2** `Viewport.renderOnDemand` — when time is paused and no parameter is changing, stop rendering entirely (battery, and it keeps fans quiet in a quiet lecture hall)
- [READY] **M2-3** `camera/` — orbit/pan/zoom wrapping `OrbitControls`, never exposed to modules
- [READY] **M2-4** `camera/` — orthographic ↔ perspective toggle, with ortho the default for any module about components, projections, or angles. Reading components off a perspective projection is misleading; this is a pedagogical requirement, not a preference (§8)
- [READY] **M2-5** `camera/` — animated presets `+X`/`+Y`/`+Z`/iso/fit-to-content, ~400 ms with an ease so students keep their spatial bearings. Instant cuts are disorienting; honour `prefers-reduced-motion` by disabling the easing (§15). Preset definitions and the "iso" orientation follow the current up axis (ADR 0009), which defaults to `y`
- [READY] **M2-6** `camera/` — camera state serializable to and restorable from the §14 compact encoding (`c=iso.o`, explicit angles once orbited), so a bookmarked demo restores the viewing angle
- [READY] **M2-7** Glyph handle protocol — every glyph is a factory returning a handle with `set(props)` / `visible(bool)` / `dispose()` that mutates buffers in place and never rebuilds (`src/scene/glyphs/Handle.ts`; §3 principle 3, §8)
- [READY] **M2-8** Glyphs, batch A (vectors and lines): `arrow` (cone head sized in **screen** space so short vectors still read as arrows; `doubleHead` for pseudovectors), `curvedArrow` (arc with a tangential head), `path` (fading tail with configurable persistence), `point` (screen-space constant size)
- [READY] **M2-9** Glyphs, batch B (areas and volumes): `patch` (correct double-sided transparency, depth-write off), `surface` (`(u,v) => Vec3`, optional scalar colouring, wireframe overlay, **and the clipping plane §8 requires** — M2 scope, not deferred: it is what later makes cross-sections and E-1/E-6 cheap), `body` (box, sphere, cylinder, disc, rod, spring helix — low poly by design)
- [READY] **M2-10** Glyphs, batch C (instruments): `arc` (optional midpoint label), `frame` (nestable triads so modules never do their own matrix bookkeeping), `axes` (**live** ticks whose labels reflect current world-unit spacing as you zoom), `graticule` (the §15 signature bezel: tick-marked scale rules along the viewport edges with live labels)
- [READY] **M2-11** `field` glyph — **instanced**, one draw call for thousands of arrows, with magnitude→length, magnitude→colour, and normalized modes. The ≤200 draw-call budget (§17) depends on this being instanced
- [READY] **M2-12** `annotate/` — `label({ latex, anchor, offset })` as a KaTeX-rendered **HTML overlay** (crisp at any zoom, selectable, accessible to screen readers), positioned by projecting the anchor each frame
- [READY] **M2-13** `annotate/` — dimension lines, dashed projection drop-lines, leader lines
- [READY] **M2-14** `theme/` — semantic palette (§15 tokens) → three materials, plus the projector variant (higher contrast, ~1.6× line weight, no subtle transparency) toggled at runtime. Modules only ever see `ctx.palette.*`, never a hex
- [READY] **M2-15** Picking — ray-cast against registered draggable handles using `kernel/geometry`'s ray intersections (M1-12). Scene exposes the hit; the _shell_ wires the drag to the param (M3-6). No mouse code in modules
- [READY] **M2-21** Up-axis support in the scene layer per ADR 0009: `ctx.up` on `SceneContext` (and mirrored in `MockSceneContext`, M2-17/M2-18), the camera up vector and presets following it, and switching reorienting through the same ~400 ms eased transition as the presets rather than cutting — `prefers-reduced-motion` respected (§15)
- [READY] **M2-16** Expand `SceneContext` to the full glyph set. It currently declares only `arrow`/`patch`/`arc` with the rest commented out — every glyph from M2-8/9/10/11 plus `label` needs a factory on the context, or modules cannot reach it
- [READY] **M2-17** `MockSceneContext` real implementation (`src/modules/testing/MockSceneContext.ts` currently throws) — headless, no WebGL, no DOM, tallying handle creates/disposes (which is what makes contract assertion 4 possible) and recording handle property sets (assertions 5 and 5b)
- [READY] **M2-18** Type-level test asserting `MockSceneContext` mirrors `SceneContext` **exactly**, in both directions, so a glyph added to one and forgotten in the other fails typecheck. This is half the M2 gate
- [READY] **M2-19** Demo scene exercising **every** glyph (replaces M0-12's throwaway cube), profiled in the Chrome performance profiler for 60 fps and zero per-frame allocation. Record the profile result in the PR so the number is reviewable rather than folklore
- [READY] **M2-20** Disposal discipline — creating and tearing down the demo scene repeatedly must not grow the WebGL context count or leak geometries/materials. Pairs with the E2E assertion in M4-8
- [READY] **M2-G** **Gate:** demo scene exercises every glyph at 60 fps with zero per-frame allocation (profiler-verified), and the `MockSceneContext` mirror test passes. Do not start M3 until this is true
