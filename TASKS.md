...
- [BLOCKED: needs the repo owner to enable Pages] **M0-14** Push to GitHub, confirm `deploy.yml` publishes to Pages at the live URL, confirm `ci.yml` is green on the PR. Pushed; found both workflows `disabled_manually` on GitHub (from the pre-lockfile pushes that failed instantly at `npm ci`), re-enabled them (`gh workflow enable`), pushed again to exercise them for real. `ci.yml` is green end to end (format check through E2E smoke). `deploy.yml`'s build+upload steps succeed but `actions/deploy-pages` fails with `404 Not Found — Ensure GitHub Pages has been enabled`: `gh api repos/SystemsDoctor/phys-viz/pages` confirms no Pages site exists yet. Per this file's own note, that's a manual repo-settings step (Settings → Pages → Source → GitHub Actions) for the repo owner, not something to attempt via API — asked rather than doing it
- [BLOCKED: M0-14] **M0-G** **Gate:** rotatable cube live at the Pages URL, CI green, cross-layer import fails lint. Do not start M1 until this is true. CI-green and the lint-boundary proof are done; the live-URL half is waiting on M0-14's Pages step
